...
// gets current value of instruction counter
...
namespace linalg
{
    ...
    {
        // unsigned __int64 before = __rdtsc(); // get starting value of instruction counter
        ...
        for (int row = 0; row < A.size(); row++)
        {
            for (int ind = 0; ind < A[0].size(); ind++)
            {
                ...
                {
                    newMat[row][col] += A[row][ind] * B[ind][col]; // Matrix i,j value is the dot product of A row and B column
                }
            }
        }

        // Benchmark
        // unsigned __int64 after = __rdtsc();
        // cout << after - before << " ticks" << endl;
...
int main()
{
    ...
    if (am3 != vector<vector<double>>{{156, 166}})
    {
